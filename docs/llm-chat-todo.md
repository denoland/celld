...
- uses vite, websockets, and openai API to demonstrate capabilities of celld
...
- [x] Add WS logic to connect to cell
...
- [x] Set `build.outDir = '../../../data/llm-chat.localhost/static'` in
      `vite.config.ts`
...
### Cell-specific env handling

- [x] Pass `OPENAI_API_KEY` to celld via env file
      (`data/llm-chat.localhost/prod.env`)
- [x] Build rust code to read env vars and pass to deno subprocess in
      `src/process_manager.rs`
- [x] Cell subprocess should read `Deno.env.get("OPENAI_API_KEY")`
- [x] Add tests for env var handling

### 4. UX polish

- [x] Distinguish bot vs. user visually (bot: gray left-align, user: blue
      right-align)

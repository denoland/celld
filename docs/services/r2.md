...
The object store keeps some keys in a percent-encoded form. This applies to a
key with a non-ASCII character, a control character, or one of the characters
``% \ { } ^ ` [ ] " < > ~ # | * ?``. For example, `přehled.html` becomes the
object `r2/<bucket_name>/p%C5%99ehled.html`. The store also keeps each empty
segment of a key as `%`, so `photos/` becomes the object
`r2/<bucket_name>/photos/%`. Therefore, the keys `a/b`, `/a/b`, `a//b`, and
`a/b/` are four different objects, as in Cloudflare R2. `list()` decodes each
key, so a listed key is the same key that `put()` received. However, `list()`
sorts the keys by the encoded form, and Cloudflare R2 sorts them by UTF-8
bytes. Therefore, a key with one of these characters can have a different
position in a celld listing. `list()` also compares the `startAfter` option
with the encoded form, so a celld listing can return a key that Cloudflare R2
skips, or skip a key that Cloudflare R2 returns. For example, Cloudflare R2
returns the key `a~` after `startAfter: "aa"`, because `~` sorts after `a`.
celld compares `a%7E` with `aa` and skips the key. A `cursor` from a celld
listing does not have this problem, because celld made it.

A tool other than celld can write an object under `r2/<bucket_name>/` with a
percent-encoded key that is not UTF-8, such as `a%FF`. No R2 key can name this
object, so `list()` fails when it reaches the object.

celld 0.5.1 and earlier removed each empty segment from a key. An object that
such a version wrote under a key like `photos/` stays under `photos`, and
`list()` returns it as `photos`.

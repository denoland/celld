...
function createTestRuntime(): {
  ...
  // Ensure workflow tables exist by calling a method that triggers table creation
  runtime.listRuns(); // This will call ensureTablesExist()
...
Deno.test("workflow type safety", () => {
  ...
  const _userSignup = runtime.define<
  ...
  const _userLogin = runtime.define<
    { userId: string },
...
Deno.test("dispatch undefined workflow throws error", () => {
  ...
  try {
    runtime.dispatch(undefinedWorkflow);
...
Deno.test("define workflow with no steps and dispatch it", async () => {
  ...
  const noStepWorkflow = runtime.define<
    { value: number },
    { processed: number }
  >({
    name: "no-step",
  ...
  const runId = runtime.dispatch(noStepWorkflow, { value: 42 });
  assertExists(runId);
...
Deno.test("define workflow with one step and dispatch it", async () => {
  ...
  const oneStepWorkflow = runtime.define<
    { value: number },
    { finalResult: number }
  >({
    name: "one-step",
  ...
  const runId = runtime.dispatch(oneStepWorkflow, { value: 42 });
  assertExists(runId);
...
Deno.test("step.run that was already executed is not executed again on retry", async () => {
  ...
  // Define a workflow
  const _workflow = runtime.define<null, string>({
    name: "workflow",
    handler: async ({ step }) => {
...
Deno.test("step.invoke enables workflow composition", async () => {
  ...
  const multiply = runtime.define<{ a: number; b: number }, number>({
    name: "multiply",
    handler: async ({ input }) => {
      await delay(1);
  ...
  const calculate = runtime.define<
    { x: number },
  ...
  const runId = runtime.dispatch(calculate, { x: 5 });
  const progress = await waitForCompletion(runtime, runId);
...
Deno.test("step.invoke recovers from crash", async () => {
  ...
  // Define workflows
  const child = runtime.define<{ a: number; b: number }, number>({
    name: "child",
    handler: async ({ input }) => {
      await delay(1);
      return input.a * input.b;
  ...
  const _parent = runtime.define<{ x: number }, { result: number }>({
    name: "parent",
    handler: async ({ input, step }) => {
...
Deno.test("step.invoke that was already executed is not executed again on retry", async () => {
  ...
  // Define workflows
  const child = runtime.define<null, string>({
    name: "child",
    handler: () => {
  ...
  const _parent = runtime.define<null, string>({
    name: "parent",
    handler: async ({ step }) => {
...
Deno.test("multiple step.invoke calls work sequentially", async () => {
  ...
  const add = runtime.define<{ a: number; b: number }, number>({
    name: "add",
    handler: async ({ input }) => {
      await delay(1);
      return input.a + input.b;
  ...
  const chain = runtime.define<{ x: number }, number>({
    name: "chain",
    handler: async ({ input, step }) => {
  ...
  const runId = runtime.dispatch(chain, { x: 10 });
  const progress = await waitForCompletion(runtime, runId);
...
Deno.test("parent down child completed recovery", async () => {
  ...
  const child = runtime.define<{ x: number }, number>({
    name: "child",
    handler: async ({ input }) => {
      await delay(1);
      return input.x * 10;
  ...
  const parent = runtime.define<{ value: number }, { processed: number }>({
    name: "parent",
    handler: async ({ input, step }) => {
  ...
  const parentRunId = runtime.dispatch(parent, { value: 5 });
...
Deno.test("void returning workflows work correctly", async () => {
  ...
  const voidWorkflow = runtime.define({
    name: "void-workflow",
    handler: async (_) => {
      await delay(1);
  ...
  const runId = runtime.dispatch(parent);
  const progress = await waitForCompletion(runtime, runId);
...
Deno.test("getRunProgress public API works correctly", async () => {
  ...
  const testWorkflow = runtime.define<{ value: number }, { result: number }>({
    name: "test-progress",
    handler: async ({ input, step }) => {
  ...
  const runId = runtime.dispatch(testWorkflow, { value: 5 });

  // Test that getRunProgress works the same as runtime.getRunProgress
  const publicProgress = runtime.getRunProgress(runId);
  ...
  const finalProgress = runtime.getRunProgress(runId);
...
Deno.test("listRuns public API works correctly", async () => {
  ...
  const workflow1 = runtime.define<{ x: number }, { result: number }>({
    name: "workflow-1",
    handler: async ({ input }) => {
      await delay(1);
  ...
  const workflow2 = runtime.define<{ y: number }, { result: number }>({
    name: "workflow-2",
    handler: async ({ input }) => {
      await delay(1);
  ...
  const runId1 = runtime.dispatch(workflow1, { x: 10 });
  const runId2 = runtime.dispatch(workflow2, { y: 20 });
  const runId3 = runtime.dispatch(workflow1, { x: 30 });
  ...
  // Test limit
  const limitedRuns = runtime.listRuns({ limit: 2 });
  assertEquals(limitedRuns.length, 2);

  // Test combined filters
  const combinedRuns = runtime.listRuns({
...
Deno.test("step.sleep suspends and resumes workflow execution", async () => {
  ...
  const sleepWorkflow = runtime.define<
    { durationMs: number },
    { message: string }
  >({
    name: "sleep-test",
  ...
  const runId = runtime.dispatch(sleepWorkflow, { durationMs: 100 });
...
Deno.test("step.sleep is idempotent on retry", async () => {
  ...
  runtime.define<{ durationMs: number }, string>({
...
Deno.test("step.sleep schedules wake-up task correctly", async () => {
  ...
  const taskSchedulingWorkflow = runtime.define<{ sleepMs: number }, null>({
  ...
  const runId = runtime.dispatch(taskSchedulingWorkflow, { sleepMs: 500 });
